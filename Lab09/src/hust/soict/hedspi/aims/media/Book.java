...
public class Book extends Media{
    ...
    public String toString() {
        String string = String.format("Type: %-10sID: %-5dTitle: %-40sCategory: %-20sCost: %10.2f$", "Book", this.getId(), this.getTitle(),this.getCategory(),this.getCost());
        if (authors.size() == 0) {
            string = "<html>" + string.replaceAll("<","&lt;").replaceAll(" ", "&nbsp;").replaceAll(">", "&gt;").replaceAll("\n", "<br/>") + "</html>";
            return string;
        }
		string += String.format("%n%20sAuthors: ","");
