...
public class AimsGUIsSwing extends JFrame {
    ...
    public AimsGUIsSwing() {
        ...
        // test media
        countMedia += 3;
        DigitalVideoDisc dvd = new DigitalVideoDisc(1, "Abcwerddd", "as", "sdf", 2, 1f);
        Book bk = new Book(5, "dfd", "ier", 4.5f);
        CompactDisc cd = new CompactDisc(3, "fdf", "we", 4.5f, "Hieu");
        order.addMedia(dvd, cd, bk);

        // display media
        displayPanel = new JPanel(new BorderLayout());
        displayPanel.add(new JLabel("Order " + order.getId(), SwingConstants.CENTER), BorderLayout.NORTH);
        listMedia = new JList<Media>(new Vector<Media>(order.getItemOrdered()));
        ...
        searchButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if (field.getText() == null) {
                    listMedia.removeAll();
                    listMedia.setListData(new Vector<Media>(order.getItemOrdered()));
                    displayPanel.revalidate();
                    displayPanel.repaint();
                    return;
                }
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new AimsGUIsSwing(); // Let the constructor do the job
            }
        });
    }
}